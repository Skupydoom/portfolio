...
- [Портфолио](#портфолио)
  ...
  - [Выполненные проекты](#выполненные-проекты)
    - [2024: Практическая работа](#2024-практическая-работа)
...
## Образование
- Обучаюсь на 4-м курсе бакалавриата МУИВ. Факультет информационных технологий.  
- С октября 2023-го года прохожу обучение в АНО "Школа 21", образовательном проекте Сбер для разработчиков.  

#### Курсы и тренинги
- 2023 Правительство Москвы, Добровольный Квалификационный Экзамен по Data Science.
- 2023 МФТИ, “NLP: автоматическая генерация текста” 72 ак. ч.
...
## Выполненные проекты
&emsp;В основном выполнял проекты, связанные с анализом данных и классическим машинным обучением. Но также углубился в NLP.  
___
### 2024: Практическая работа
**“Классификация постов пользователей на несколько эмоций только по одному признаку: тексту сообщения”**  
<a href="https://github.com/Skupydoom/portfolio/tree/main/source/Классификация%20постов%20пользователей%20на%20несколько%20эмоций%20только%20по%20одному%20признаку%20-%20тексту%20сообщения">Ссылка</a> на исходный код и отчёт.  

**Задачи:**  
-	Подготовить среду разработки, настроить конфигурацию оборудования;
-	Изучить аспекты создания нейронных сетей в области NLP;
-	Подобрать подходящую архитектуру нейросети для достижения цели работы;
-	Провести предварительный анализ данных;
-	Обработать и нормализовать данные;
-	Разделить данные на тренировочные и тестовые.
-	Привести данные в необходимый для нейросети вид:
    -	Создать словарь и загрузчик данных по частям;
    -	Преобразовать тексты в тензоры PyTorch;
-	Написать нейронную сеть, выбрать оптимизатор;
-	Добавить функции обучения и валидации;
-	Оценить эффективность модели машинного обучения, сделать выводы;
-	Сделать попытку улучшить результат путём изменения архитектуры сети, подбора гиперпараметров;
-	Резюмировать проделанную работу, составить рекомендации для решения задач NLP в рамках исследования социальных сетей

**Используемые технологии:**  
- Python, PyTorch, Scikit-learn, Spacy, Wordninja, Numpy, Pandas, Matplotlib, Seaborn, Jupyter Notebook.

**Результаты:**  
-	Настроил оборудование для эффективной работы с обучением нейросетей и токенизацией текста с использованием графической карты для ускорения вычислений;
-	Исследовал существующие архитектуры нейронных сетей для достижения задач анализа предметной области;
-	Выбрал и математически описал подходящую архитектуру нейросети для данных.
-	Предобработал и векторизовал данные;
-	Создал все необходимые для библиотеки PyTorch классы, такие как: словарь, загрузчик данных, нейросеть;
-	Реализовал нейросеть по ранее смоделированной архитектуре;
-	Улучшил через графики и отладочную информацию изначальную модель, проверил точность на тестовых данных.
- Сделал выводы по исследованию предметной области.


**Описание:**  
&emsp;В проекте исследовал датасет, содержащий только тексты постов пользователей социальной сети и метки эмоционального окраса.  
&emsp;В ходе работы провёл полный анализ и обработку исходных данных, построил наглядные графики.
&emsp;Описал математически и реализовал архитектуру нейронной сети.
&emsp;Улучшил через графики и отладочную информацию изначальную модель, проверил точность на тестовых данных.
...
- Создание простого контейнера Docker для сервера, работа с Dockle, Nginx и Docker Compose.
- Настройка сети на виртуальных машинах с Linux и их взаимодействие.
- Базовое ознакомление с Linux (Ubuntu Server LTS 20.04).
- Реализация библиотеки C для обработки матриц.
